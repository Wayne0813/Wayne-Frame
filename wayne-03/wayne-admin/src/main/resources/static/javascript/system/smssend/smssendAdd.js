layui.config({
    ...
    form.on('select(sms-config)', function (data) {
        ...
        var val = data.value;
        if (!val) {
            var $s = $('select[name="SMS_TEMPLATE_ID"]');
            $s.empty();
            return;
        }
        ...
        //去后台查询出改配置对应的所有的模版
        admin.callAjaxPost({
            url: "/smsconfig/listTemplateNoPage.do",
            data: {SMS_CONFIG_ID: val},
            success: function (result) {
                ...
                if (result.success && result.code === 200) {
                    ...
                    for (var i = 0; i < result.data.length; i++) {
                        var opt = '<option value="' + result.data[i].ID + '" data-params="' + result.data[i].TEMPLATE_PARAMS + '">' + result.data[i].TEMPLATE_NAME + '</option>';
                        $s.append(opt);
                    }
                    form.render();
                } else {
    ...
    form.on('select(sms-config-template)', function (data) {
        ...
        $('textarea[name="MESSAGE"]').attr("placeholder", "请填写以下参数：" + params);
    });
