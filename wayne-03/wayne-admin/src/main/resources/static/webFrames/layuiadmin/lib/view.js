...
layui.define(['laytpl', 'layer'], function (exports) {
    ...
    /**
     * 发起ajax post json格式请求
     *
     * @param options ajax请求业务参数和配置参数
     */
    view.callAjaxPost = function (options) {
        var that = this,
            request = setter.request,
            response = setter.response,
            debug = function () {
        ...
        for (var item in  options) {
            if (item in ajaxConfig) {
                ajaxConfig[item] = options.config
            }
        }

        options.config = ajaxConfig || {
            dataType: 'json',
            type: 'post',
            async: true,
            contentType: 'application/json',
        ...
        if (request.tokenName) {
            //自动给 Request Headers 传入 token
            options.config.headers[request.tokenName] = request.tokenName in options.config.headers
                ? options.config.headers[request.tokenName]
                : (layui.data(setter.tableName)[request.tokenName] || '');
        }
        ...
        return $.ajax({
            ...
            dataType: options.config.dataType, //服务器返回json格式数据
            type: options.config.type, //HTTP请求类型
            async: options.config.async,
            contentType: options.config.contentType,
            timeout: options.config.timeout, //超时时间设置为10秒；
            headers: options.config.headers,
            success: function (result) {
                var statusCode = response.statusCode;
                //登录状态失效，清除本地 access_token，并强制跳转到登入页
                if (result[response.statusName] === statusCode.logout || result[response.statusName] === statusCode.invalidToken) {
                    view.exit(function () {
                        var error = [
                            '<cite>Code：</cite> ' + (result[response.statusName] || '返回状态码异常') + '<br><cite>Error：</cite> ' + (result[response.msgName] || '返回状态码异常'),
                            debug()
                        ].join('');
                        view.error(error);
                    });
                ...
                //对于其他错误，直接提示
                else if (result[response.statusName] === statusCode.exception || result[response.statusName] === statusCode.notFound || result[response.statusName] === statusCode.exception || result[response.statusName] === statusCode.noResponse) {
                    var error = [
                        '<cite>Code：</cite>' + (result[response.statusName] || '返回状态码异常') + '<br><cite>Error：</cite>' + (result[response.msgName] || '返回状态码异常'),
                        debug()
                    ].join('');
            ...
            error: function (e, code) {
                console.log(e);
                var error = [
                    '<cite>错误编号：</cite>' + e.responseJSON.status + '<br><cite>错误信息：</cite>' + e.responseJSON.message
                    , debug()
                ].join('');
                view.error(error);
    ...
    view.msgtemp = function (code, msg) {
        return error = [
            '<cite>状态：</cite>' + code + '<br><cite>信息：</cite>' + msg
        ].join('');
    };
