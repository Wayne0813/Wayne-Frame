layui.config({
    ...
    var EMAIL_CONFIG_ID = $('input[name="EMAIL_CONFIG_ID"]').val();
    ...
    var myTable = table.render({
        elem: '#grid-table',
        url: '/emailconfig/listTemplate.do',
        method: 'post',
        where: {data: {EMAIL_CONFIG_ID: EMAIL_CONFIG_ID}},
        ...
        cols: [[
            ...
            {field: 'TEMPLATE_TOPIC', title: '模板主题'},
            {field: 'TEMPLATE_SENDER', title: '发送人昵称'},
            {field: 'TEMPLATE_CONTENT', title: '邮件内容'},
    ...
    table.on('tool(grid-table)', function (obj) {
        ...
        if (obj.event === 'edit') {
            ...
            layer.open({
                type: 2,
                title: '编辑公告',
                content: '/emailconfig/emailConfigTemplateAdd.html?ID=' + data.ID + '&EMAIL_CONFIG_ID=' + EMAIL_CONFIG_ID,
                resize: false,
                area: ['550px', '550px'],
                btn: ['确定', '取消'],
            ...
            batchEnable({IDS: [data.ID], TEMPLATE_STATUS: 1, EMAIL_CONFIG_ID: EMAIL_CONFIG_ID})
        } else if (obj.event === 'disable') {
            batchDisable({IDS: [data.ID], TEMPLATE_STATUS: 0, EMAIL_CONFIG_ID: EMAIL_CONFIG_ID})
        } else if (obj.event === 'del') {
            batchDel({IDS: [data.ID], EMAIL_CONFIG_ID: EMAIL_CONFIG_ID})
        }
    });
    ...
    var toolBarActive = {
        ...
        add: function () {
            layer.open({
                type: 2,
                title: '新增配置',
                content: '/emailconfig/emailConfigTemplateAdd.html?EMAIL_CONFIG_ID=' + EMAIL_CONFIG_ID,
                resize: false,
                area: ['550px', '550px'],
                btn: ['确定', '取消'],
                yes: function (index, layero) {
        ...
        batchenable: function () {
            ...
            var params = {
                IDS: ids, TEMPLATE_STATUS: 1, EMAIL_CONFIG_ID: EMAIL_CONFIG_ID
            };
            batchEnable(params);
        },
        //批量禁用
        batchdisable: function () {
            ...
            var params = {
                IDS: ids, TEMPLATE_STATUS: 0, EMAIL_CONFIG_ID: EMAIL_CONFIG_ID
            };
            batchDisable(params);
        },
        //批量删除
        batchdel: function () {
            ...
            var params = {
                IDS: ids, EMAIL_CONFIG_ID: EMAIL_CONFIG_ID
            };
            batchDel(params);
        }
    };
                ...
                success: function (result) {
                    ...
                    if (result.success && result.code === 200) {
                        ...
                        var param = {
                            ...
                            TEMPLATE_TOPIC: $('input[name="TEMPLATE_TOPIC"]').val(),
    ...
    function batchDisable(data) {
        layer.confirm('确定禁用这些模版吗？', function (index) {
            ...
            //调用接口
            admin.callAjaxPost({
                url: '/emailconfig/disableTemp.do',
                data: data,
                success: function (result) {
                    //1、判断状态
                    layer.close(lTip);
