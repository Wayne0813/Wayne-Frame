layui.config({
    ...
    table.on('tool(grid-table)', function (obj) {
        ...
        if (obj.event === 'edit') {
            ...
            layer.open({
                type: 2,
                title: '编辑公告',
                content: '/smssend/smssendAdd.html?ID=' + data.ID,
                resize: false,
                area: ['550px', '550px'],
                btn: ['确定', '取消'],
    ...
    var toolBarActive = {
        ...
        send: function () {
            layer.open({
                type: 2,
                title: '新增配置',
                content: '/smssend/smssendAdd.html',
                resize: false,
                area: ['550px', '550px'],
                btn: ['确定', '取消'],
                yes: function (index, layero) {
    ...
    function batchResend(data) {
        layer.confirm('确定重发这些短信吗？', function (index) {
            layer.close(index);
            //调用ajax删除
            //请求登入接口
                ...
                success: function (result) {
                    ...
                    if (result.success && result.code === 200) {
                        ...
                        var param = {
                            ...
                            KEY_WORDS: $('input[name="SMS_SERVER_NAME"]').val(),
                            PUB_STATUS: $('select[name="SEND_STATUS"]').val()
                        };
                        myTable.reload('grid-table', {
                            where: {data: param}
                        });
    ...
    function batchsend(data) {
        layer.confirm('确定发送这些短信吗？', function (index) {
            layer.close(index);
            //调用ajax删除
            //请求登入接口
            var lTip = layer.load(5, {time: 1000, shade: 0.1});
